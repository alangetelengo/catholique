...
@section('title', 'Nouveau type de revenu - Catholique')
@section('page-title', 'Nouveau type de revenu')
@section('page-title-info', 'Créer un type de recette rattaché à une catégorie.')
...
@section('content')
    <div class="adventiste-card-pro-static p-6">
        <form method="post" action="{{ route('revenue-types.store') }}" class="grid grid-cols-1 md:grid-cols-2 gap-5" id="revenue-type-form">
            @csrf
            @if (auth()->user()?->hasRole('super_admin'))
                <div class="md:col-span-2">
                    ...
                    <p class="mt-1.5 text-xs text-slate-500 dark:text-slate-400">Seules les catégories de cette paroisse sont proposées ci-dessous.</p>
                </div>
            @else
                <input type="hidden" name="paroisse_id" id="rt_paroisse_hidden" value="{{ auth()->user()?->paroisse_id }}">
            @endif
            <div class="md:col-span-2">
