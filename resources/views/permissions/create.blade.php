...
@section('title', 'Créer une permission — Catholique')
                ...
                <form action="{{ route('permissions.store') }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input id="libelle_permission" type="text" name="libelle_permission" value="{{ old('libelle_permission') }}"
                        ...
                        <div>
                            ...
                            <input id="name" type="text" name="name" value="{{ old('name') }}"
                                ...
                                required autocomplete="off" placeholder="ex. view_members">
                            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1.5">Souvent <code class="text-[11px]">verbe_ressource</code> (snake_case).</p>
                            @error('name')
                                <p class="text-xs text-red-600 dark:text-red-400 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="flex flex-wrap items-center gap-3 pt-2 border-t border-slate-200/80 dark:border-slate-700/80">
                        <button type="submit" class="adventiste-btn-primary">Créer la permission</button>
        ...
        <aside class="lg:col-span-1">
            <div class="adventiste-card-pro-static p-5 sm:p-6 border border-sky-200/40 dark:border-sky-900/30 bg-sky-50/40 dark:bg-sky-950/20">
                <h2 class="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2 mb-3">
                    <i class="fas fa-key text-sky-600 dark:text-sky-400" aria-hidden="true"></i>
                    Bonnes pratiques
                </h2>
                <ul class="text-sm text-slate-600 dark:text-slate-400 space-y-2 list-disc list-inside">
                    <li>Nom unique, en minuscules avec underscores.</li>
                    <li>Associez la permission aux rôles depuis l’onglet <strong class="text-slate-800 dark:text-slate-200">Rôles</strong>.</li>
                    <li>Évitez de supprimer une permission encore assignée.</li>
                </ul>
            </div>
        </aside>
    </div>
