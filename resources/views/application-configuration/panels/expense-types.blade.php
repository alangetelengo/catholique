@php
    $configTab = 'expense-types';
@endphp
<div class="application-config-panel space-y-4" data-config-tab="{{ $configTab }}">
    @if ($canManage)
        <div class="flex flex-wrap items-center justify-between gap-3">
            <a href="{{ route('expense-types.create') }}" class="adventiste-btn-primary text-sm no-underline">+ Nouveau type</a>
        </div>
    @endif
                ...
                <tbody class="divide-y divide-slate-200/70 dark:divide-slate-700/70">
                    @forelse ($types as $type)
                        <tr>
                            ...
                            <td class="px-4 py-3">
                                <div class="font-medium">{{ $type->nom }}</div>
                                @if ($type->description)
                                    <div class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{{ $type->description }}</div>
                                @endif
                            </td>
                            ...
                            @if ($canManage)
                                <td class="px-4 py-3">
                                    <div class="flex justify-end gap-2">
                                        <x-action-button
                                            variant="edit"
                                            href="{{ route('expense-types.edit', $type) }}"
                                        ...
                                        <x-action-button
                                            variant="delete"
                                            action="{{ route('expense-types.destroy', $type) }}"
                                            method="DELETE"
                                            confirm-message="Supprimer ce type de dépense ?"
                                        />
                                    </div>
                                </td>
                            @endif
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ $canManage ? 5 : 4 }}" class="px-4 py-8 text-center text-slate-500">Aucun type de dépense.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
