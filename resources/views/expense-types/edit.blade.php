...
@section('title', 'Modifier type de dépense — Catholique')
@section('page-title', 'Modifier type de dépense')
@section('page-title-info', 'Mise à jour du type de dépense.')
...
@section('content')
    <div class="adventiste-card-pro-static p-6">
        <form method="post" action="{{ route('expense-types.update', $type) }}" class="grid grid-cols-1 md:grid-cols-2 gap-5">
            @csrf
            @method('put')
            <div>
